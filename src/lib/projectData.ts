...
export function normalizeProject(row: UserProjectRow): NormalizedProject {
  ...
  const characters = normalizeCollection<Character>(
    ...
    (item, index) => ({
      ...
      // desktop field names
      appearance: asString(item.appearance || item.physicalDescription || item.physical_description),
      background: asString(item.background || item.backstory),
      secrets: asString(item.secrets),
      desires: asString(item.desires || item.motivations),
      other: asString(item.other),
      image: asString(item.image),
      // legacy mobile fields kept for backwards compat
      role: asString(item.role || item.type),
      description: asString(item.description || item.notes),
      age: asString(item.age),
      physicalDescription: asString(item.physicalDescription || item.physical_description || item.appearance),
      backstory: asString(item.backstory || item.background),
      motivations: asString(item.motivations || item.desires),
  ...
  return {
    ...
    publishedChapterIds: asStringArray(raw.publishedChapterIds || raw.published_chapter_ids || raw.publishedChapters),
...
export function denormalizeProject(project: NormalizedProject): JsonRecord {
  return {
    ...
    characters: project.characters.map((character) => ({
      ...
      age: character.age || '',
      physicalDescription: character.physicalDescription || character.appearance || '',
      physical_description: character.physicalDescription || character.appearance || '',
      backstory: character.backstory || character.background || '',
      motivations: character.motivations || character.desires || '',
